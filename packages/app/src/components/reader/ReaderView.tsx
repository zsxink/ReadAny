...
export function ReaderView({ bookId, tabId }: ReaderViewProps) {
  ...
  useEffect(() => {
    ...
    void foliateRef.current?.setTTSHighlight(currentTTSSegment?.cfi || null, "gray");
    ...
    const continuationCallback = async () => {
      ...
      const tryContinue = async (attempt = 0) => {
        ...
        if (text && ttsContinuousRef.current) {
          ...
          void primeDesktopTTSLyricContext(dedupedSegments[0]?.cfi || firstCfi);
  ...
  const startPageTTS = useCallback(
    async (continuous = ttsContinuousEnabled) => {
      ...
      setShowTTS(true);
      void primeDesktopTTSLyricContext(normalizedSegments[0]?.cfi || readerTab?.currentCfi || "");
